...
@app.post("/skills/execute", dependencies=[Depends(require_auth)])
async def execute_skill(request: dict):
    """
    Directly execute a finance skill for the auto-trade bot.
    Used for real-time SMC, Elliott Wave, or Trend confirmation.
    """
    from src.agent.skills import SkillsLoader

    skill_name = request.get("skill")
    symbol = request.get("symbol", "UNKNOWN")
    data = request.get("data") # List of OHLC dicts
    
    loader = SkillsLoader()
    skill = next((s for s in loader.skills if s.name == skill_name), None)
