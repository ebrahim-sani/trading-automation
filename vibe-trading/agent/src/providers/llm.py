...
def _sync_provider_env() -> None:
    ...
    _PROVIDER_MAP: dict[str, tuple[str | None, str | None]] = {
        "openai":     ("OPENAI_API_KEY",     "OPENAI_BASE_URL"),
        "openrouter": ("OPENROUTER_API_KEY",  "OPENROUTER_BASE_URL"),
        "deepseek":   ("DEEPSEEK_API_KEY",    "DEEPSEEK_BASE_URL"),
        "gemini":     ("GOOGLE_API_KEY",      None), # Standard native gemini env
        "google":     ("GOOGLE_API_KEY",      None), # Alternative name
    ...
    if api_key:
        ...
        os.environ["GOOGLE_API_KEY"] = api_key # Also sync for native provider
    if base_url:
...
def build_llm(*, model_name: Optional[str] = None, callbacks: Any = None) -> Any:
    ...
    if provider in ("gemini", "google") and ChatGoogleGenerativeAI is not None:
