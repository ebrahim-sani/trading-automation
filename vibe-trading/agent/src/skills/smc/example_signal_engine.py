            ...
            for target, aliases in col_map.items():
                ...
                for alias in aliases:
                    if alias in df.columns:
                        target_cols[target] = df[alias]
                        found = True
                        break
            
                if not found:
                    if target == "volume":
                        # Fallback for volume-less feeds
                        target_cols["volume"] = pd.Series(1, index=df.index)
                    else:
                        raise KeyError(f"Required column '{target}' missing from data. Found: {list(df.columns)}")

            ohlc = pd.DataFrame(target_cols).copy()
